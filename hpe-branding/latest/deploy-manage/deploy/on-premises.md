...
# metadata # 
title:  On Premises
description: Learn how to deploy MLDM on-premises. 
date: 
# taxonomy #
tags: ["deployment"]
series:
...
This page walks you through the fundamentals of what you need to know about Kubernetes, persistent volumes, and object stores to deploy MLDM on-premises.

{{% notice info %}}
- Read our [infrastructure recommendations](../ingress/). You will find instructions on how to set up an ingress controller, a load balancer, or connect an Identity Provider for access control. 
- If you are planning to install MLDM UI. Read our [Console deployment](../console/) instructions. Note that, unless your deployment is `LOCAL` (i.e., on a local machine for development only, for example, on Minikube or Docker Desktop), the deployment of Console requires, at a minimum, the set up of an Ingress.
- Troubleshooting a deployment? Check out [Troubleshooting Deployments](../../../troubleshooting/deploy-troubleshooting).
{{% /notice %}}

{{% notice tip %}}
We are now shipping MLDM with an **optional embedded proxy**  allowing your cluster to expose one single port externally. This deployment setup is optional.
...
1. Finally, [Deploy MLDM using Helm](../helm-install) by running the `helm install` command with the appropriate values configured in your values.yaml. We recommend reading these generic deployment steps if you are unfamiliar with Helm.
...
Do not deploy an on-premises MLDM cluster against cloud-based object stores (such as S3, GCS, Azure Blob Storage). 
...
Once you have Kubernetes deployed, your storage classes setup, and your object store configured, follow those steps [to Helm install MLDM on your cluster](../helm-install).
