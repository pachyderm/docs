...
A [datum](../) is a MLDM abstraction that helps in optimizing
...
When new data comes in (in the form of commit(s) in its input repo(s)), a MLDM pipeline automatically starts a new job. Each MLDM job consists of the
following stages:

### 1: Creation of input datums
In this stage, MLDM **creates datums based on the input data according to the
...
`pfs/out` output directory that MLDM 
...
MLDM detects three datums. However, because `datum 1` and `datum 3` are
unchanged, it skips processing these datums. MLDM detects
...
using the `pachctl put file` command, MLDM will
