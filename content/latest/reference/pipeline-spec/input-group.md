...
|`repo`|Specifies the name of the MLDM repository that contains the input data.|
|`branch`| The branch to watch for commits. If left blank, MLDM sets this value to master. |
| `glob`| A wildcard pattern that defines how a dataset is broken up into datums for further processing. When you use a glob pattern in a group input, it creates a naming convention that MLDM uses to group the files.|
...
The `group` input in a MLDM Pipeline Spec allows you to group input files by a specific  pattern. 
...
You can specify multiple group input fields in a MLDM Pipeline Spec, each with their own group_by parameter. This allows you to group files by multiple fields or patterns, and pass each group to your pipeline as a separate datum.
