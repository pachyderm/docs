...
1. Run the corresponding steps for your operating system:  
   ...
   * For a **Debian-based Linux 64-bit or Windows 10 or later running on WSL** (Choose the command matching your architecture):  
     ...
     - ARM Architectures (arm64):
       ...
       ```s  
       curl -o /tmp/pachctl.deb -L https://github.com/pachyderm/pachyderm/releases/download/v{{% majorMinorVersion %}}.x/pachctl_{{% majorMinorVersion %}}_arm64.deb && sudo dpkg -i /tmp/pachctl.deb  
       ```  
   ...
   * For all **other Linux flavors** (Choose the command matching your architecture):  
     ...
     - ARM Architectures (arm64):
       ...
       ```s  
       curl -o /tmp/pachctl.tar.gz -L https://github.com/pachyderm/pachyderm/releases/download/v{{% majorMinorVersion %}}.x/pachctl_{{% majorMinorVersion %}}_linux_arm64.tar.gz && tar -xvf /tmp/pachctl.tar.gz -C /tmp && sudo cp /tmp/pachctl_{{% majorMinorVersion %}}_linux_arm64/pachctl /usr/local/bin 
       ```  
